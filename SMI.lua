...
function main()
	...
	checkUpdate() -- Проверка на обновление скрипта
...
function checkUpdate()
   postrequest = {}
   postrequest.data = "n="..GetNickName().."&i="..select(1, sampGetCurrentServerAddress())
   postrequest.headers = {
       ['content-type']='application/x-www-form-urlencoded'
   } 
	asyncHttpRequest("POST", "https://github.com/cparkMM/mass_master/blob/master/mm_update.php", postrequest,
		function(response)
         if updateInfo ~= nil then
   			local updateInfo = decodeJson(response.text)
            if tonumber(updateInfo['version_number']) ~= thisScript().version_num then
               sampAddChatMessage("Была найдена новая версия: "..updateInfo['version']..'. Начинаю установку..')
               download_id = downloadUrlToFile(updateInfo['url'], thisScript().path, download_handler_update)
            end
         end
		end,
		function(err)
			sampAddChatMessage("Произошла ошибка при обновлении..")
		end)
end


function download_handler_update(id, status, p1, p2)
  if status == dlstatus.STATUS_DOWNLOADINGDATA then
    print(string.format('Загружено %d из %d.', p1, p2))
  elseif status == dlstatus.STATUS_ENDDOWNLOADDATA then
   sampAddChatMessage("Установка завершена!")
    thisScript():reload()
  end
end
